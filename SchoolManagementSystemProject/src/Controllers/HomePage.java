...
public class HomePage extends JFrame implements ActionListener {
	...
	public void addStudentRowToJtable() {
		...
		int stdCnt = 1;
		for (int i = 0; i < rj.s.studentArr.size(); i++) {
			rowData[0] = stdCnt++;
			rowData[1] = rj.s.studentArr.get(i).getSurname();
			rowData[2] = rj.s.studentArr.get(i).getName();
			rowData[3] = rj.s.studentArr.get(i).getGrade();
			rowData[4] = rj.s.studentArr.get(i).getAge();
			rowData[5] = rj.s.studentArr.get(i).getFee();
	...
	public void addTeacherRowToJtable() {
		...
		for (int i = 0; i < rj.t.teacherArr.size(); i++) {
			rowData[0] = rj.t.teacherArr.get(i).getSurname();
			rowData[1] = rj.t.teacherArr.get(i).getName();
			rowData[2] = rj.t.teacherArr.get(i).getBranch();
			rowData[3] = rj.t.teacherArr.get(i).getSalary();
	...
	public void addClassRowToJtable() {
		...
		Object rowData[] = new Object[6];
		int cnt_grade = 1;
		for (int i = 0;  i < rj.c.classArr.size(); i++) {
			rowData[0] = cnt_grade++;  // grade id
			rowData[1] = rj.c.classArr.get(i).getMondayCourse();
			rowData[2] = rj.c.classArr.get(i).getTuesdayCourse();
			rowData[3] = rj.c.classArr.get(i).getWednesdayCourse();
			rowData[4] = rj.c.classArr.get(i).getThursdayCourse();		
			rowData[5] = rj.c.classArr.get(i).getFridayCourse();
	...
	public void addTeacherToArryList(String surname, String name, String salary, String bracnh_type) {
		...
		rj.t.teacherArr.add(new_teacher);
		// teacher add new bracnh, update class edit and add page combo box values.
		int ind = rj.t.teacherArr.size();   ind--;
		jtClassEditRowData.comboBox_mday.addItem(rj.t.teacherArr.get(ind).getBranch()); 
		jtClassEditRowData.comboBox_tuesday.addItem(rj.t.teacherArr.get(ind).getBranch());
		jtClassEditRowData.comboBox_wday.addItem(rj.t.teacherArr.get(ind).getBranch());
		jtClassEditRowData.comboBox_thursday.addItem(rj.t.teacherArr.get(ind).getBranch());
		jtClassEditRowData.comboBox_fday.addItem(rj.t.teacherArr.get(ind).getBranch());
		jtClassAddRowData.comboBox_mday.addItem(rj.t.teacherArr.get(ind).getBranch()); 
		jtClassAddRowData.comboBox_tuesday.addItem(rj.t.teacherArr.get(ind).getBranch());
		jtClassAddRowData.comboBox_wday.addItem(rj.t.teacherArr.get(ind).getBranch());
		jtClassAddRowData.comboBox_thursday.addItem(rj.t.teacherArr.get(ind).getBranch());
		jtClassAddRowData.comboBox_fday.addItem(rj.t.teacherArr.get(ind).getBranch());
	...
	public void cleanTextFieldClassInfo() {	
		jtClassAddRowData.comboBox_mday.setSelectedItem(rj.t.bracnhArr.get(0));
		jtClassAddRowData.comboBox_fday.setSelectedItem(rj.t.bracnhArr.get(0));
		jtClassAddRowData.comboBox_wday.setSelectedItem(rj.t.bracnhArr.get(0));
		jtClassAddRowData.comboBox_thursday.setSelectedItem(rj.t.bracnhArr.get(0));
		jtClassAddRowData.comboBox_tuesday.setSelectedItem(rj.t.bracnhArr.get(0));
	}	
	...
	public void addStudentToArryList(String surname, String name, String grade, String age, String fee) {
		...
		for(int i=0; i<rj.sb.schoolBusArr.size(); i++) {
			int busCapacity = Integer.parseInt(rj.sb.schoolBusArr.get(i).getCapacity());
			if( rj.sb.schoolBusArr.get(i).getStudentsIDs().size() < busCapacity ) {
				s = new ArrayList<String>();
				for(int j=0; j<rj.sb.schoolBusArr.get(i).getStudentsIDs().size(); j++) {
					int schoolbusStdID = Integer.parseInt(rj.sb.schoolBusArr.get(i).getStudentsIDs().get(j));
					if(schoolbusStdID == rj.std_cnt )  // if student have a bus. 
						flag = true;
					else { s.add(rj.sb.schoolBusArr.get(i).getStudentsIDs().get(j)); }
				}
		...
		if( capacity == true && flag == false ) {// student have no in any bus and bus capacity have. 
			rj.sb.schoolBusArr.get(whicFreeBusIndx).setStudentsIDs(s);
	...
	public boolean checkHaveSameBracnhType(String bracnh_type) {
		for(int i=0; i<rj.t.teacherArr.size(); i++) {
			if(bracnh_type.equals(rj.t.teacherArr.get(i).getBranch().toLowerCase())) {
	...
	public void addStudent() {
		...
		if ((!surname.equals("") && !name.equals("") && !grade.equals("") && !age.equals("") && !fee.equals(""))) {
			if (rj.s.checkHaveSameStudent(surname, name)) {
	...
	public void addClass() {
		...
		if ((!mday.equals("") && !tuesday.equals("") && !wday.equals("") && !thursday.equals("") && !fday.equals(""))) {
			if (rj.c.checkHaveSameClass(mday, tuesday, wday, thursday, fday)) {
	...
	public void addTeacher() {
		...
		if ((!surname.equals("") && !name.equals("") && !bracnh_type.equals("") && !salary.equals(""))) {
			if (rj.t.checkHaveSameTeacher(surname, name)) {
	...
	public void updateClass() {
		...
		if ((!monday.equals("") && !tuesday.equals("") && !wday.equals("") && !thursday.equals("") && !friday.equals(""))) {
			if (rj.c.checkHaveSameClass(monday, tuesday, wday, thursday, friday)) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == btnDetailsStudent_main) {  // Main panels buttons
			...
			if(row_index != -1) {  // if selected specific row from student table, Show details page
				...
				std_index--;
				String name = rj.s.studentArr.get(std_index).getName();
				String surname = rj.s.studentArr.get(std_index).getSurname();
				...
				jtStudentDetailsRowData.textField_mondayCourse.setText( rj.c.classArr.get(grade_index).getMondayCourse());
				jtStudentDetailsRowData.textField__tuesdayCourse.setText( rj.c.classArr.get(grade_index).getTuesdayCourse());
				jtStudentDetailsRowData.textField_wednesdayCourse.setText( rj.c.classArr.get(grade_index).getWednesdayCourse());
				jtStudentDetailsRowData.textField_thursdayCourse.setText( rj.c.classArr.get(grade_index).getThursdayCourse());
				jtStudentDetailsRowData.textField_fridayCourse.setText( rj.c.classArr.get(grade_index).getFridayCourse());
				...
				for(SchoolBus schBus : rj.sb.schoolBusArr) {					
	...
	// Create the frame.	
		public void HomePageView() { 
			...
			gl_studentInfo.setHorizontalGroup(gl_studentInfo.createParallelGroup(Alignment.LEADING).addGroup(gl_studentInfo.createSequentialGroup().addContainerGap()
			    .addComponent(btnDetailsStudent_main, GroupLayout.DEFAULT_SIZE, 159, Short.MAX_VALUE).addPreferredGap(ComponentPlacement.RELATED)
				.addComponent(btnUpdateStudent_main, GroupLayout.DEFAULT_SIZE, 159, Short.MAX_VALUE).addPreferredGap(ComponentPlacement.RELATED)
				.addComponent(btnAddStudent_main, GroupLayout.DEFAULT_SIZE, 180, Short.MAX_VALUE).addGap(7)).addComponent(lbl_topicStudent, GroupLayout.DEFAULT_SIZE, 
				529, Short.MAX_VALUE).addGroup(gl_studentInfo.createSequentialGroup()
				.addContainerGap().addComponent(scrollPane_std, GroupLayout.DEFAULT_SIZE, 510, Short.MAX_VALUE).addGap(7)));
			gl_studentInfo.setVerticalGroup(gl_studentInfo.createParallelGroup(Alignment.LEADING).addGroup(gl_studentInfo.createSequentialGroup().addGap(14)
				.addComponent(lbl_topicStudent).addGap(18).addGroup(gl_studentInfo.createParallelGroup(Alignment.BASELINE, false).addComponent(btnDetailsStudent_main)
				.addComponent(btnUpdateStudent_main).addComponent(btnAddStudent_main)).addGap(18).addComponent(scrollPane_std, GroupLayout.DEFAULT_SIZE, 206, 
			...
			gl_teacherInfo.setHorizontalGroup(gl_teacherInfo.createParallelGroup(Alignment.TRAILING).addComponent(lbl_topicTeacher, GroupLayout.DEFAULT_SIZE, 525, Short.MAX_VALUE)
				.addGroup(gl_teacherInfo.createSequentialGroup().addGap(99).addComponent(btnUpdateTeacher_main, GroupLayout.DEFAULT_SIZE, 159, Short.MAX_VALUE)
				.addGap(6).addComponent(btnAddTeacher_main, GroupLayout.DEFAULT_SIZE, 180, Short.MAX_VALUE).addGap(81)).addGroup(Alignment.LEADING, 
				gl_teacherInfo.createSequentialGroup().addContainerGap().addComponent(scrollPane_tch, GroupLayout.DEFAULT_SIZE, 501, Short.MAX_VALUE).addContainerGap()));
			gl_teacherInfo.setVerticalGroup(gl_teacherInfo.createParallelGroup(Alignment.TRAILING).addGroup(Alignment.LEADING, gl_teacherInfo.createSequentialGroup()
				.addContainerGap().addComponent(lbl_topicTeacher).addPreferredGap(ComponentPlacement.UNRELATED).addGroup(gl_teacherInfo.createParallelGroup(Alignment.LEADING)
				.addComponent(btnUpdateTeacher_main).addComponent(btnAddTeacher_main)).addPreferredGap(ComponentPlacement.UNRELATED).addComponent(scrollPane_tch, 
				GroupLayout.DEFAULT_SIZE, 216, Short.MAX_VALUE).addContainerGap()));
			...
			gl_contentPane.setHorizontalGroup(gl_contentPane.createParallelGroup(Alignment.LEADING).addGroup(gl_contentPane.createSequentialGroup().addContainerGap()
				.addComponent(lbl_ruleProcess2, GroupLayout.DEFAULT_SIZE, 538, Short.MAX_VALUE)
				.addContainerGap()).addComponent(lbl_smsTopic, GroupLayout.DEFAULT_SIZE, 562, Short.MAX_VALUE)
				.addGroup(gl_contentPane.createSequentialGroup().addContainerGap().addComponent(tabbedPane).addContainerGap()));
			gl_contentPane.setVerticalGroup(gl_contentPane.createParallelGroup(Alignment.LEADING).addGroup(gl_contentPane.createSequentialGroup().addContainerGap()
				.addComponent(lbl_smsTopic, GroupLayout.PREFERRED_SIZE, 39, GroupLayout.PREFERRED_SIZE).addGap(18).addComponent(tabbedPane, GroupLayout.DEFAULT_SIZE, 342,
				Short.MAX_VALUE).addGap(7).addComponent(lbl_ruleProcess2, GroupLayout.PREFERRED_SIZE, 16, GroupLayout.PREFERRED_SIZE).addContainerGap()));
